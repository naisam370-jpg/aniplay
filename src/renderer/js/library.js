...
// Make ipcRenderer available globally to avoid duplicate imports
...
class AniPlayApp {
    ...
    async scanLibrary() {
        this.showLoading(true, 'Scanning library and fetching from MyAnimeList...');
        ...
        try {
            ...
            message += `📊 Results:\n`;
            message += `• Added: ${result.added || 0} new anime\n`;
            message += `• Processed: ${result.processed}/${result.total} folders\n`;
            message += `• Errors: ${result.errors.length}\n`;
            
            if (result.errors.length > 0) {
                message += `\n❌ Errors:\n`;
                result.errors.forEach(error => {
                    message += `• ${error.folder}: ${error.error}\n`;
                });
            }
        ...
        return `
            <div class="anime-card" data-anime-id="${anime.id}">
                <div class="anime-cover">
                    ${coverPath 
                        ? `<img src="${coverPath}" alt="${anime.title}" loading="lazy" onerror="this.parentElement.innerHTML='<div class=\\"cover-placeholder\\">🎌</div>'">` 
